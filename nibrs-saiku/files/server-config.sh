...
curl -sSL https://raw.githubusercontent.com/ojbc/analytics/master/nibrs/NIBRSAnalyticsMondrianSchema.xml -o /tmp/nibrs_analytics.xml
...
curl -s -u admin:admin -F name="nibrs_analytics" -F file="@/tmp/nibrs_analytics.xml;filename=nibrs_analytics.xml;type=text/xml" http://localhost/saiku/rest/saiku/admin/schema/nibrs_analytics/


echo "Installing data sources..."

curl -s -X POST -u admin:admin --header "Content-Type: application/json" -T /tmp/nibrs-analytics.connection.json http://localhost/saiku/rest/saiku/admin/datasources
